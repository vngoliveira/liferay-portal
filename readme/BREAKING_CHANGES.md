...
When reading, treat `description` as a map keyed by language ID (for example, `{"en_US": "Sends the welcome email."}`) rather than a string, and select the entry for the language you want. When writing, send the same map shape; a plain string is no longer accepted. In a site initializer, write the property the way `errorMessage` and `label` are already written, as an object keyed by language ID, because a plain string no longer parses. Existing stored descriptions are migrated to the company's default language by an upgrade process, so no data is lost. Two behaviors that are not visible in the type change are worth knowing. A `PUT` that omits `description`, or sends an empty map, no longer clears the stored value: the empty map resolves to null and the stored description survives, where a request that omitted the property used to clear it. Send `{"en_US": ""}` to clear a language explicitly. And an object action that has no description omits the property from the response rather than returning an empty map, which is what the plain string did before this change.
